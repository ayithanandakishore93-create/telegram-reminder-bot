...
def parse_reminder(text: str):
    """
    Fast local parser for common reminder phrases.
    This handles the most common natural language formats reliably.
    """
    t = clean_text(text)

    patterns = [
        # remind me in 1 minute to drink water
        (r"remind me in (\d+)\s*(minute|minutes|min|hour|hours|hr|hrs)\s*(?:to\s+)?(.+)", False, False),
        # remind me to drink water in 1 minute
        (r"remind me(?: to)? (.+?) in (\d+)\s*(minute|minutes|min|hour|hours|hr|hrs)", True, False),
        # remind me every 10 minutes to drink water
        (r"remind me every (\d+)\s*(minute|minutes|min|hour|hours|hr|hrs)\s+to\s+(.+)", False, True),
        # remind me to drink water every 10 minutes
        (r"remind me(?: to)? (.+?) every (\d+)\s*(minute|minutes|min|hour|hours|hr|hrs)", True, True),
        # remind me at 11:30 am to call mom
        (r"remind me at (\d{1,2})(?::(\d{2}))?\s*(am|pm)\s+to\s+(.+)", False, False),
        # remind me to call mom at 11:30 am
        (r"remind me(?: to)? (.+?) at (\d{1,2})(?::(\d{2}))?\s*(am|pm)", True, False),
        # remind me tomorrow at 9am to study
        (r"remind me tomorrow(?: at)? (\d{1,2})(?::(\d{2}))?\s*(am|pm)\s+to\s+(.+)", False, False),
        # remind me to study tomorrow at 9am
        (r"remind me(?: to)? (.+?) tomorrow(?: at)? (\d{1,2})(?::(\d{2}))?\s*(am|pm)", True, False),
    ]
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 3) Chat
    try:
        if route.get("intent") == "chat" and route.get("reply"):
            reply = route["reply"]
        else:
            reply = ai_chat(text)
    except Exception:
